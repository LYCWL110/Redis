...
static HNode **h_lookup(HTab *htab, HNode *key,
...
const size_t kResizingWork = 128;
...
static void hm_help_resizing(HMap *hmap) {
  ...
  size_t nwork = 0;
  while (nwork < kResizingWork && hmap->ht2.size > 0) {
...
HNode *HmLookup(HMap *hmap, HNode *key,
                bool (*cmp)(HNode *, HNode *)) {
...
HNode *HmPop(HMap *hmap, HNode *key,
             bool (*cmp)(HNode *, HNode *)) {
  hm_help_resizing(hmap);
  HNode **from = h_lookup(&hmap->ht1, key, cmp);
  if (from) {
...
size_t HmSize(HMap *hmap) { return hmap->ht1.size + hmap->ht2.size; }
